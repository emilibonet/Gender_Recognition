...
from DSFD.data import widerface_640
...
class MultiBoxLoss(nn.Module):
        ...
        if pa and self.odm and self.use_pa:
            ...
            if refine:
                face_loss_l, face_loss_c = self.part_forward(
                    ...
                    (predictions[-2], predictions[-1]), True)
            else:
                face_loss_l, face_loss_c = self.part_forward(
                    (predictions[0], predictions[1], predictions[2]), targets)
            self.part = 'head'
    ...
    def part_forward(self,
        ...
        for idx in range(num):
            ...
            if ac:
                sfd_match(self.threshold, truths, defaults, self.variance,
...
class focalLoss(MultiBoxLoss):
    ...
    def part_forward(self,
        ...
        p_t_log = -F.cross_entropy(
            conf_p, conf_targets[pos_cls], size_average=False)
